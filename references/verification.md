...
- For an SVG state control, capture off, hover, and active states. Confirm the icon is visible and centered in its complete hit target and that the exact `image-color` changes. Use real pointer input when posted window messages do not reproduce backend hit testing or plug-in behavior.
- For an interactive VST editor, instrument the process bridge with processed-block, missed-block, and longest-miss-run counters. Open a real plug-in GUI, click its virtual keyboard, assert nonzero audio output, and require no misses during the measured window after warm-up. A screenshot proves the window state but cannot prove continuous audio.
